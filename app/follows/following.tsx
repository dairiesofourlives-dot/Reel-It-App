...
export default function FollowingScreen() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        // who targetUserId is following
        const { data } = await supabase
          .from('follows')
          .select('followee_id, profiles!inner(user_id, username, avatar_url)')
        ...
        console.log('Load following failed', e);
      }
    };
  ...
  return (
    <View style={[commonStyles.wrapper, styles.container]}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 30 }}>
        {rows.length === 0 ? (
          <Text style={styles.empty}>No following accounts to show.</Text>
        ) : (
          rows.map((r) => {
            ...
            return (
              <View key={r.user_id} style={styles.row}>
                ...
                >
                  <Text style={[styles.followTxt, following && { color: colors.text }]}>{following ? 'Following' : 'Follow'}</Text>
                </Pressable>
              </View>
            );
          })
